...
3. Για τον συγχρονισμό με conditional variables χρησιμοποιήσαμε επίσης *NTHREADS* εξ αυτών. Η μόνη διαφορά με τους σημαφόρους είναι πως
   ...
   οδηγώντας σε αχρείαστα context switches εφόσον όλα εκτός από ένα θα έμπαιναν απευθείας σε κατάσταση αναμονής. Παρακάτω φαίνεται η διαφορά μεταξύ
των δυο στρατηγικών:

	Χρήση ενός conditional variable για κάθε νήμα (4 νήματα):
   ...
   ```
	...
	Χρήση ενός conditional variable για όλα τα νήματα (4 νήματα):
...
4. Προφανώς, το κρίσιμο τμήμα πρέπει να περιλαμβάνει μόνο την εκτύπωση, αφού μόνο εκείνη χρειάζεται συγχρονισμό (σειριοποίηση).
   ...
   sys.....0m1,549s
	...
	Παρατηρούμε πως ο χρόνος εκτέλεσης είναι αντιστρόφως ανάλογος (προσεγγιστικά) με τον αριθμό των νημάτων, μέχρι τα νήματα να φτάσουν να
...
μεταξύ όλων των νημάτων, αφού κάποια δε βρίσκουν διαθέσιμο πυρήνα για να εκτελεσθούν ταυτόχρονα. Έχουμε δηλαδή ταυτοχρονισμό, ή σειριακή εκτέλεση, 
η οποία λόγω του κόστους του συγχρονισμού ίσως οδηγεί σε μικρή αύξηση του συνολικού χρόνου εκτέλεσης. 
